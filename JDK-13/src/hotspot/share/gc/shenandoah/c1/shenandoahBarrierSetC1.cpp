...
void ShenandoahBarrierSetC1::prefetch_barrier(LIRGenerator* gen, LIR_Opr obj, CodeEmitInfo* info, DecoratorSet decorators, bool need_null_check) {
  if (UseShenandoahGC && ShenandoahPrefetchBarrier) {
    prefetch_barrier_impl(gen, obj, info, decorators, need_null_check);
  }
}

void ShenandoahBarrierSetC1::prefetch_barrier_impl(LIRGenerator* gen, LIR_Opr obj, CodeEmitInfo* info, DecoratorSet decorators, bool need_null_check) {
...
LIR_Opr ShenandoahBarrierSetC1::storeval_barrier(LIRGenerator* gen, LIR_Opr obj, CodeEmitInfo* info, DecoratorSet decorators) {
  if (ShenandoahStoreValEnqueueBarrier) { // MemLiner: Default false
    obj = ensure_in_register(gen, obj);
...
void ShenandoahBarrierSetC1::load_at_resolved(LIRAccess& access, LIR_Opr result) {
  ...
  prefetch_barrier(access.gen(), result, access.access_emit_info(), access.decorators(), true);
...
void ShenandoahBarrierSetC1::generate_c1_runtime_stubs(BufferBlob* buffer_blob) {
  ...
  _prefetch_barrier_c1_runtime_code_blob = Runtime1::generate_blob(buffer_blob, -1, 
                                                              "shenandoah_prefetch_barrier_slow",
