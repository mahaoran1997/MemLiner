...
void PrefetchQueue::handle_completed_buffer() {
  ShouldNotCallThis();
}

PrefetchQueue::PrefetchQueue(PrefetchQueueSet* qset) :
  ...
  PtrQueue(qset, false /* active is set to true*/),
...
bool PrefetchQueueSet::apply_closure_to_completed_buffer(PrefetchBufferClosure* cl) {
  BufferNode* nd = NULL;
  {
    ...
    if (_completed_buffers_head != NULL) {
      ...
      if (_n_completed_buffers == 0) _process_completed_buffers = false;
    }
  }
  if (nd != NULL) {
